...
namespace Nirast.Pcms.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [AllowAnonymous]
        [HttpPost]
        public ActionResult ChangePassword(ChangePassword changePasswordInputs)
        {
            ...
            if (ModelState.IsValid)
            {
                if (Session["loginName"] != null)
                {
                    api = "Home/RetrievePassword";
                    ForgotPasswordViewModel model = new ForgotPasswordViewModel();
                    model.Email = changePasswordInputs.EmailId;
                    var serviceContent = JsonConvert.SerializeObject(model);
                    var serviceResult = service.PostAPIWithData(serviceContent, api);
                    ...
                    changePasswordInputs.NewPassword = StringCipher.Encrypt(changePasswordInputs.NewPassword, encryptionPassword);
                    serviceContent = JsonConvert.SerializeObject(changePasswordInputs);
                    HttpStatusCode result = service.PostAPI(serviceContent, api);
                    Session.Clear();
                    changePasswordInputs = new ChangePassword();
            ...
            return View(changePasswordInputs);
        }
        //
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            ...
            {
                Service service = new Service();
                var api = "User/GetCheckLoginNameAlreadyExist/" + model.Email + "/";
                var result = service.GetAPI(api);
                if (Convert.ToInt32(result) == 2)
                {
                    ViewBag.Error = "Inactive User. Please contact administrator at " + System.Configuration.ConfigurationManager.AppSettings["FromEmail"].ToString();
                    return View();
                }
